...
slReadEDF <- function(mydata) {
  #parse the data into 24 hours and 10 minutes intervals
  thresh <- nrow(mydata$timings)/144
  ...
  for(i in 0:143){
    ...
    myAvg3EEG1<- mean(hold3Hz$EEG.EEG1A.B)
    myAvg10EEG1<- mean(hold10Hz$EEG.EEG1A.B)
    ###find the mean of hold3Hz and hold8Hz for the second electrode
    myAvg3EEG2 <- mean(hold3Hz$EEG.EEG2A.B)
    myAvg10EEG2 <- mean(hold10Hz$EEG.EEG2A.B)
  ...
  coolEDFTable <- data.frame(time=c(1:23))
  coolEDFTable["EEG1"]<- NA
  coolEDFTable["EEG2"]<- NA
  coolEDFTable["EEG1"]<- avgavg1
  coolEDFTable["EEG2"]<- avgavg2
...
ui <- fluidPage(
  ...
  navbarPage("BSEEG Calculator", id = "del",
             ...
             "),
                          ...
                          actionButton('submit',   strong(em('Submit')), 
                                       style="color: #000000; background-color: #CFB53B; border-color: #CFB53B")
                        ),
                        mainPanel(
                          h4(strong("Instructions:")),
                          p("1. Please upload 24-hour EEG recordings of mouse in order using the buttons on the left.", strong("If you keep getting disconnected from the server try waiting for one file to fully upload before uploading the next one.")),
                          p("2. Click on the submit button and wait for the results to appear on page.You can upload as many files as you want.", em("Alternatively you can switch to the 'Standardized BSEEG Plot' tab without clicking the submit button.")),
             ...
             tabPanel("BSEEG Plot", title = "Standardized BSEEG Plot", value = "BSEEG Plot", 
                      ...
                      tabsetPanel(
                        id="myGrid",
                        tabPanel("day 1", DT::dataTableOutput("cool0") %>% withSpinner(color="#CFB53B")
                                 ),
             ...
             navbarMenu("more",
                        ...
                        tabPanel("Contact Us", 
                                 ...
                                 p("For other questions or errors regarding the app please contact", em("annice-najafi@uiowa.edu"))
...
server <- function(input, output, session){
  ...
  output$normalizedPlot <- renderPlot({
    ...
    if(input$EEG1 && input$EEG2){
      df %>% ggplot(aes(x=x, y=y1), xaxt='n') + geom_line(aes(color = 'EEG1'), size=1)+ geom_line(aes(y=y2, color ='EEG2'), size=1) +
      ...
      df %>% ggplot(aes(x = x, y=y1)) + geom_line(color = 'red', size=1) +
        scale_y_continuous(name="Standardized BSEEG Score", limits=input$ylim) + ggtitle("Standardized BSEEG") + scale_x_continuous(name="Time (12 hr)", limits=c(1, normBack()[[2]]),  breaks = normBack()[[11]],labels=normBack()[[10]]
                                                                                                                                    #, labels=normBack()[[10]]
                                                                                                                                    ) +
        theme(text = element_text(size=15))
      ...
      df %>% ggplot(aes(x = x, y=y2)) + geom_line(color = 'blue', size=1) +
        scale_y_continuous(name="Standardized BSEEG Score", limits=input$ylim) + ggtitle("Standardized BSEEG") + scale_x_continuous(name="Time (12 hr)", limits=c(1, normBack()[[2]]),  breaks = normBack()[[11]],labels=normBack()[[10]]
                                                                                                                                    #, labels=normBack()[[10]]
                                                                                                                                    ) +
        theme(text = element_text(size=15))
  ...
  output$gridDiurnal<- renderPlot({
    ...
    myPlot0 <- df0 %>% ggplot(aes(x = x, y=y10)) + geom_line(aes(color = 'EEG1'), size=1)+ geom_line(aes(y=y20, color ='EEG2'), size=1)+ scale_x_continuous(name="Time (Hr)") +
      scale_y_continuous(name="BSEEG Score", limits=input$ylimBase) + ggtitle("BSEEG - Baseline Day")
    coolTable1 <- c(NA)
    ...
    myPlot1 <- df1 %>% ggplot(aes(x = x, y=y11)) + geom_line(aes(color = 'EEG1'), size=1)+ geom_line(aes(y=y21, color ='EEG2'), size=1)+ scale_x_continuous(name="Time (Hr)") +
      scale_y_continuous(name="BSEEG Score", limits=input$ylimBase) + ggtitle("BSEEG - Day 2")
    coolTable2 <- c(NA)
    ...
    myPlot2 <- df2 %>% ggplot(aes(x = x, y=y12)) + geom_line(aes(color = 'EEG1'), size=1)+ geom_line(aes(y=y22, color ='EEG2'), size=1)+ scale_x_continuous(name="Time (Hr)") +
      scale_y_continuous(name="BSEEG Score", limits=input$ylimBase) + ggtitle("BSEEG - Day 3")
    coolTable3 <- c(NA)
    ...
    myPlot3 <- df3 %>% ggplot(aes(x = x, y=y13)) + geom_line(aes(color = 'EEG1'), size=1)+ geom_line(aes(y=y23, color ='EEG2'), size=1)+ scale_x_continuous(name="Time (Hr)") +
      scale_y_continuous(name="BSEEG Score", limits=input$ylimBase) + ggtitle("BSEEG - Day 4")
    coolTable4 <- c(NA)
    ...
    myPlot4 <- df4 %>% ggplot(aes(x = x, y=y14)) + geom_line(aes(color = 'EEG1'), size=1)+ geom_line(aes(y=y24, color ='EEG2'), size=1)+ scale_x_continuous(name="Time (Hr)") +
      scale_y_continuous(name="BSEEG Score", limits=input$ylimBase) + ggtitle("BSEEG - Day 5")
    coolTable5 <- c(NA)
    ...
    myPlot5 <- df5 %>% ggplot(aes(x = x, y=y15)) + geom_line(aes(color = 'EEG1'), size=1)+ geom_line(aes(y=y25, color ='EEG2'), size=1)+ scale_x_continuous(name="Time (Hr)") +
      scale_y_continuous(name="BSEEG Score", limits=input$ylimBase) + ggtitle("BSEEG - Day 6")
    coolTable6 <- c(NA)
    ...
    myPlot6 <- df6 %>% ggplot(aes(x = x, y=y16)) + geom_line(aes(color = 'EEG1'), size=1)+ geom_line(aes(y=y26, color ='EEG2'), size=1)+ scale_x_continuous(name="Time (Hr)") +
      scale_y_continuous(name="BSEEG Score", limits=input$ylimBase) + ggtitle("BSEEG - Day 7")
  ...
  normBack <- eventReactive(input$submit,{
    ...
    dataday0 <- import_raw(holdedf$datapath)
    #use the written function slReadEDF to find the BSEEG score
    coolTable0 <- slReadEDF(dataday0)
    rm(dataday0)
    #add a column to coolTable0 including time
    coolTable0$time <- c(-18:4)
    ...
    return(list(normalizedTable, limlim, coolTable0, coolTable1, coolTable2, coolTable3, coolTable4, coolTable5, coolTable6, labelHold, breakHold))
  },ignoreNULL = FALSE)
